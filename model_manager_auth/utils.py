...
def get(data, what, delimeter=":"):
    """simple recursive search in dict by delimeter
    """
    w = what.split(delimeter)
    has_key = False
    ...
    if len(w) > 1 and has_key:
        return get(result, delimeter.join(w[1:]))
    elif len(w) == 1 and has_key:
        return result
    return None