...
class BaseClient(PaginatedManager):
    ...
    def set_api(self):
        self.api = '%s://%s:%s%s' % (
            getattr(settings, "DEVOPS_PORTAL_PROTOCOL", "http"),
            getattr(settings, "DEVOPS_PORTAL_HOST", "127.0.0.1"),
            getattr(settings, "DEVOPS_PORTAL_PORT", 9755),
            getattr(settings, "DEVOPS_PORTAL_API_PREFIX", "/api"))
